        ...
        <div class="dash-main">
          <div class="card week-strip">
            ...
            <div class="ws-days">
              <div v-for="d in weekDays" :key="d.iso"
                   class="ws-day"
                   :class="{ today: d.isToday, selected: d.iso === selectedDate, 'has-task': d.hasTasks }"
          ...
          <div class="filter-bar">
            <button v-for="f in filters" :key="f.key"
                    class="fchip" :class="{ active: activeFilter===f.key }"
          ...
          <TransitionGroup name="task-list" tag="div" class="task-list">
            <div v-for="t in filteredTasks" :key="t.id"
                 class="card task-item" :class="{ done: t.done }">
              ...
              <div class="task-acts">
                <button class="tact-del" @click="tasks.deleteTask(t.id)">✕</button>
              </div>
            </div>
          </TransitionGroup>
          ...
          <div v-if="filteredTasks.length === 0" class="empty-state">
            ...
            <button class="btn btn-primary" style="margin-top:12px;" @click="showModal=true">Add your first task</button>
          </div>

          <button class="add-row" @click="showModal=true">+ Add new task</button>
        </div>
          ...
          <div class="card side-card">
            ...
            <div class="stat-grid">
              <div class="stat-block">
                <div class="sn" style="color:var(--accent)">{{ tasks.totalDone }}</div>
                <div class="sl">Done</div>
              </div>
              <div class="stat-block">
                <div class="sn" style="color:#E05A4E">{{ tasks.totalPending }}</div>
                <div class="sl">Pending</div>
              </div>
              <div class="stat-block">
                <div class="sn" style="color:#D4933C">{{ tasks.highPriority }}</div>
                <div class="sl">High priority</div>
              </div>
              <div class="stat-block">
                <div class="sn" style="color:#5A9ACA">{{ tasks.tasks.length }}</div>
          ...
          <div class="card side-card">
            ...
            <div class="deadline-list">
              <div v-for="t in upcomingDeadlines" :key="t.id" class="dl-item">
                ...
                <span class="pill" :class="ppill[t.priority]">{{ formatDate(t.due) }}</span>
              </div>
              <p v-if="!upcomingDeadlines.length" style="font-size:12px;color:var(--muted)">No upcoming deadlines.</p>
            </div>
          </div>

          <button class="btn btn-primary" style="width:100%;" @click="showModal=true">+ Add task</button>
        </aside>
      </div>
    </div>
  </div>
...
const filteredTasks = computed(() => {
  ...
  switch(activeFilter.value) {
    case 'selected':   list = list.filter(t => t.due === selectedDate.value); break
    ...
    case 'incomplete': list = list.filter(t => !t.done); break
  }
  return list
})
...
.wsnav { background:none;border:1px solid var(--border2);color:var(--muted2);width:26px;height:26px;border-radius:5px;cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:center;transition:all .15s; }
.wsnav:hover { color:var(--text);border-color:var(--muted); }
.ws-days { display:grid;grid-template-columns:repeat(7,1fr);gap:3px; }
.ws-day { display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 4px;border-radius:8px;cursor:pointer;transition:all .15s;position:relative; }
.ws-day:hover { background:var(--surface2); }
.ws-day.selected { background:var(--surface3);border:1px solid var(--border2); }
.ws-day.today { background:var(--accent); }
.wsd-dow { font-size:10px;font-weight:500;color:var(--muted);letter-spacing:.04em; }
.ws-day.today .wsd-dow { color:rgba(255,255,255,.7); }
.wsd-num { font-size:14px;font-weight:600;color:var(--muted2); }
.ws-day.today .wsd-num { color:#fff; }
.wsd-dot { width:4px;height:4px;border-radius:50%;background:var(--accent); }
.ws-day.today .wsd-dot { background:rgba(255,255,255,.6); }
...
.task-item { padding:12px 14px;display:flex;align-items:center;gap:12px;transition:all .15s;cursor:default;border-radius:8px; }
